...
public class TensorOperationsProvider {
    ...
    private static final boolean forceJOCL = Boolean.getBoolean("jlama.force_jocl_tensor_operations");
    ...
    private TensorOperations pickFastestImplementation() {
        ...
        // Try JOCL OpenCL GPU backend FIRST — auto-detects GPU, no flag needed
        if (!forcePanama && !forceSimd) {
            try {
                Class<? extends TensorOperations> joclClazz = (Class<? extends TensorOperations>) Class.forName(
                    "com.github.tjake.jlama.tensor.operations.opencl.JOCLTensorOperations"
                );
                TensorOperations jocl = joclClazz.getConstructor().newInstance();
                java.lang.reflect.Method isAvailable = joclClazz.getMethod("isAvailable");
                if ((Boolean) isAvailable.invoke(jocl)) {
                    pick = jocl;
                    logger.info("GPU detected — using JOCL OpenCL for inference acceleration.");
                }
            } catch (Throwable t) {
                logger.debug("JOCL GPU backend not available: {}", t.getMessage());
            }
        }

        if (!forcePanama && pick == null) {
            if (!forceSimd) {
                try {
